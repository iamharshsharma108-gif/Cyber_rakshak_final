...
def public_landing():
    ...
    selected=st.selectbox("Language / भाषा / மொழி",list(lang_options),index=list(lang_options).index(lang_name),key="public_language",label_visibility="collapsed")
    if lang_options[selected]!=current:
        st.session_state.lang=lang_options[selected]; st.rerun()
...
def auth_screen():
    ...
    current=st.session_state.get("lang","en")
    lang_name=next((k for k,v in lang_options.items() if v==current),"English")
    selected=st.selectbox("Language / भाषा / மொழி",list(lang_options),index=list(lang_options).index(lang_name),key="auth_language",label_visibility="collapsed")
    if lang_options[selected]!=current:
        st.session_state.lang=lang_options[selected]; st.rerun()
    st.markdown(r"""
    <style>
